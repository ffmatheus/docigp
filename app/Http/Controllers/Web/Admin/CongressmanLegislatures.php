...
class CongressmanLegislatures extends Controller
{
     ...
     */
    public function __construct(CongressmanLegislaturesRepository $congressmanLegislaturesRepository)
    {
    ...
    public function removeFromLegislature(Request $request)
    {
        ...
        $this->congressmanLegislaturesRepository->remmoveFromLegislature($request['congressman_id'],$request['ended_at']);

        return redirect()->route('congressmen.index')->with($this->getSuccessMessage("Removido da Legislatura com Sucesso"));

    }

    public function includeInLegislature(Request $request)
    {
        $this->congressmanLegislaturesRepository->includeInLegislature($request['congressman_id'],$request['started_at']);
