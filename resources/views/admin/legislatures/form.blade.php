...
@section('content')
    <div class="card card-default" id="vue-legislatures">
        <form name="formulario" id="formulario" @if($mode == 'show') action="{{ route('legislatures.update', ['id' => $legislature->id]) }}" @else action="{{ route('legislatures.store')}}" @endIf method="POST">
            {{ csrf_field() }}
